...
> **Нужно больше мощности?** Смотри раздел [RunPod](#runpod--альтернатива-colab) — облачные GPU без лимитов Colab, включая API для автоматизации.
...
| **Img2Img** (Wan 2.2 + Lightning) | Стилизация фото за 4 шага + апскейл x2 ClearReality | `colab_wan_img2img` |
| **Dance Transfer** (SteadyDancer) | Видео с танцем + фото → персонаж танцует (детекция поз ViTPose) | `colab_wan_dancer` |
...
| `colab_wan_img2img.ipynb` | Wan 2.2 Img2Img — стилизация фото. Lightning LoRA (4 шага = быстро!) + апскейл x2 ClearReality. Выход: 2560x3840. | [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/russiankendricklamar/comfyui-colab-toolkit/blob/main/notebooks/colab_wan_img2img.ipynb) |
| `colab_wan_dancer.ipynb` | SteadyDancer — перенос танца. Видео с танцем + фото → персонаж танцует. Детекция поз ViTPose. Выход: 720x1280 @ 30fps. | [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/russiankendricklamar/comfyui-colab-toolkit/blob/main/notebooks/colab_wan_dancer.ipynb) |
...
- **«Хочу стилизовать фото?»**
  → [`colab_wan_img2img.ipynb`](https://colab.research.google.com/github/russiankendricklamar/comfyui-colab-toolkit/blob/main/notebooks/colab_wan_img2img.ipynb) *(Wan 2.2 img2img + Lightning 4 шага + апскейл)*

- **«Хочу перенести танец на персонажа?»**
  → [`colab_wan_dancer.ipynb`](https://colab.research.google.com/github/russiankendricklamar/comfyui-colab-toolkit/blob/main/notebooks/colab_wan_dancer.ipynb) *(SteadyDancer, детекция поз)*
...
### 7. Wan 2.2 Img2Img (`colab_wan_img2img.ipynb`)

Стилизация фотографий с помощью Wan 2.2 + Lightning LoRA.

#### Ключевые особенности

- **Lightning LoRA** — генерация за 4 шага (вместо 20–30), в 5–7 раз быстрее
- **UltimateSDUpscale** — апскейл x2 (1280x1920 → 2560x3840) с 4x ClearReality
- **Гибкий denoise** — от лёгкой стилизации (0.3) до полной переработки (0.9)

#### Процесс

1. Загрузите исходное фото (портрет, пейзаж, арт — всё подходит)
2. В ComfyUI выберите фото в ноде LoadImage
3. Напишите промпт с желаемым стилем
4. Queue Prompt → результат за ~1–2 мин + ~2–3 мин на апскейл

| Параметр | Значение | Описание |
|----------|----------|----------|
| denoise | 0.3–0.9 | Сила изменений |
| steps | 4 | Lightning = 4 шага |
| resolution | 1280x1920 | Портрет |
| upscale | x2 | ClearReality 4x |

**Модели:** ~15 ГБ (Wan 2.2 fp8 + UMT5 + VAE + Lightning LoRA + апскейлер)

---

### 8. SteadyDancer (`colab_wan_dancer.ipynb`)

Перенос танцевальных движений на любого персонажа.

#### Как работает

```
Видео с танцем  ──→  [ViTPose + YOLO]  ──→  Скелет поз
                                                  ↓
Фото персонажа  ──→  [CLIP + VAE]     ──→  [SteadyDancer]  ──→  Видео с танцем
```

1. ViTPose извлекает скелет человека из каждого кадра видео
2. CLIP Vision кодирует внешность персонажа с фото
3. SteadyDancer генерирует видео где персонаж повторяет те же движения
4. Звук из оригинального видео переносится в результат

#### Требования к входным данным

| Файл | Рекомендации |
|------|-------------|
| **Видео с танцем** | 1 человек, полная фигура, стабильная камера, 5–15 сек, .mp4 |
| **Фото персонажа** | В полный рост или по пояс, нейтральная поза, чёткое фото |

| Параметр | Значение |
|----------|----------|
| Выход | 720x1280 @ 30fps, H.264 MP4 |
| Шаги | 4 (LightX2V LoRA) |
| Block swap | 20 (для T4) |

**Модели:** ~20 ГБ (SteadyDancer fp8 + UMT5 bf16 + VAE + CLIP Vision + LoRA + ViTPose + YOLO)

**Важно:** SteadyDancer использует ~15 ГБ VRAM — T4 (16 ГБ) работает на пределе. Если OOM: уменьшите num_frames или увеличьте block_swap.
...
| `photo_wan_img2img.json` | Wan 2.2 + Lightning | Photo | фото + текст | стилизованное | 4 шага + апскейл x2 |
| `video_wan_dancer.json` | Wan 2.1 SteadyDancer | Dance | видео танца + фото | танец персонажа | Детекция поз ViTPose |
...
## RunPod — альтернатива Colab

Google Colab бесплатен, но имеет ограничения: сессия до 12 часов, случайные отключения, 16 ГБ VRAM (T4), лимит GPU-квоты. **RunPod** — облачный GPU по запросу, без этих лимитов.

### Зачем RunPod?

| | Google Colab (free) | RunPod Pod | RunPod Serverless |
|---|---|---|---|
| Стоимость | Бесплатно | от $0.20/час | Оплата за секунду |
| Сессия | Макс. 12ч | Без ограничений | По запросу |
| GPU | Только T4 (16 ГБ) | Любая GPU | Любая GPU |
| Хранилище | Google Drive | Network Volume | Network Volume |
| API | Нет | Нет (UI) | Полноценный REST API |
| Для кого | Начинающие, тесты | Продвинутые пользователи | Автоматизация, продакшн |

### Таблица GPU на RunPod

| GPU | VRAM | ~Цена/час | Макс. разрешение | Макс. кадры | blocks_to_swap |
|---|---|---|---|---|---|
| T4 | 16 ГБ | $0.20 | 832x480 | 481 (20 сек) | 20 |
| RTX 3090 | 24 ГБ | $0.22 | 1024x576 | 601 (25 сек) | 5 |
| RTX 4090 | 24 ГБ | $0.44 | 1280x720 | 721 (30 сек) | 0 |
| L4 | 24 ГБ | $0.24 | 1024x576 | 601 (25 сек) | 10 |
| A40 | 48 ГБ | $0.39 | 1280x720 | 721 (30 сек) | 0 |
| A100 | 80 ГБ | $0.79 | 1280x720 | 721 (30 сек) | 0 |
| H100 | 80 ГБ | $1.99 | 1280x720 | 721 (30 сек) | 0 |

> Цены приблизительные и могут отличаться. Актуальные цены на [runpod.io](https://www.runpod.io/pricing).

---

### Pod Quick Start (интерактивный режим)

Работает как Colab, но мощнее и без лимитов по времени. Запускаете Pod → получаете ComfyUI UI в браузере.

**Шаг 1.** Создайте Pod на [runpod.io](https://www.runpod.io/console/pods):
- Template: `RunPod PyTorch 2.x`
- GPU: выберите из таблицы выше
- Volume: 50 ГБ (для моделей)

**Шаг 2.** Откройте терминал Pod (кнопка Terminal) и выполните:

```bash
git clone https://github.com/russiankendricklamar/comfyui-colab-toolkit
bash comfyui-colab-toolkit/runpod/pod/setup.sh
```

**Шаг 3.** Скрипт автоматически:
- Определит GPU и подберёт оптимальные настройки
- Установит ComfyUI + все кастомные ноды
- Скачает модели (~20 ГБ, 10–15 мин)
- Скопирует все 14 воркфлоу
- Запустит ComfyUI

**Шаг 4.** Откройте URL из вывода скрипта:
```
https://{POD_ID}-8188.proxy.runpod.net
```

> **Совет:** Создайте Network Volume и сохраните модели в `/runpod-volume/models/`.
> При следующем запуске скрипт создаст симлинки вместо скачивания — старт за секунды.

---

### Serverless Quick Start (API-режим)

Для автоматизации: отправляете JSON-запрос → получаете видео/фото. Оплата только за время генерации.

**Шаг 1.** Соберите Docker-образ:

```bash
cd comfyui-colab-toolkit/runpod/serverless

# Скопируйте API-воркфлоу в директорию сборки
mkdir -p workflows
cp ../../workflows/*_api.json workflows/

# Полный образ с моделями (~25 ГБ, cold start ~30 сек)
docker build -t comfyui-serverless .

# ИЛИ лёгкий образ без моделей (~5 ГБ, нужен Network Volume)
# Закомментируйте COPY --from=downloader строки в Dockerfile
```

**Шаг 2.** Запушьте образ:

```bash
docker tag comfyui-serverless your-username/comfyui-serverless:latest
docker push your-username/comfyui-serverless:latest
```

**Шаг 3.** Создайте Serverless Endpoint на [runpod.io](https://www.runpod.io/console/serverless):
- Container Image: `your-username/comfyui-serverless:latest`
- GPU: A40 или лучше (рекомендуется)
- Max Workers: по вашим потребностям

**Шаг 4.** Отправляйте запросы:
...
curl -X POST "https://api.runpod.ai/v2/YOUR_ENDPOINT_ID/runsync" \
  ...
  -d '{
    "input": {
      ...
      "prompt": "кот в космосе, cinematic, 4K quality",
      "params": {"width": 832, "height": 480, "num_frames": 81}
    }
  }'

# Картинка → Видео (base64)
curl -X POST "https://api.runpod.ai/v2/YOUR_ENDPOINT_ID/runsync" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "input": {
      "workflow": "video_wan_clip",
      "prompt": "девушка поворачивает голову и улыбается",
      "images": [{"name": "face.jpg", "image": "'$(base64 -i face.jpg)'"}],
...
curl -X POST "https://api.runpod.ai/v2/YOUR_ENDPOINT_ID/runsync" \
  ...
  -d '{
    "input": {
      "workflow": "photo_flux_instagram",
      "prompt": "professional portrait, studio lighting, magazine cover"
    }
  }'
```

### Доступные workflow для Serverless API

| Имя | Описание | Требуется изображение |
|---|---|---|
| `video_wan_t2v` | Текст → Видео | Нет |
| `video_wan_clip` | Картинка → Видео | Да |
| `video_wan_v2v` | Видео → Видео (стилизация) | Да |
| `video_wan_talking` | Говорящая голова | Да + аудио |
| `video_wan_long_ultimate` | Длинное видео (до 30 сек) | Опционально |
| `video_wan_reels` | Вертикальное видео для Reels | Да (папка) |
| `video_wan_dancer` | Танцующий персонаж | Да |
| `video_hunyuan_t2v` | HunyuanVideo T2V | Нет |
| `video_cogvideo_t2v` | CogVideoX T2V | Нет |
| `photo_flux_instagram` | Фото для Instagram | Да (папка) |
| `photo_flux_img2img` | Стилизация фото | Да |
| `photo_wan_img2img` | Img2Img через Wan | Да |
...
|---|---|---|
| Максимальная сессия | ~12 часов | Бесплатный Colab (RunPod — без лимита) |
| Таймаут простоя | ~90 минут | Держите вкладку активной |
| Память GPU | 16 ГБ (T4) | `blocks_to_swap` для больших моделей |
| Макс. длина видео | 30 сек (721 кадр) | 832x480 на T4, больше на RunPod GPU |
...
| Квота GPU | Ограничена | Google может ограничить (RunPod — нет) |
